...
(function () {
  ...
  const BUNNY_DEFAULT_TASKS = [
    {id:1,category:"Produksjon",name:"Stekte tomater",amount:3,amount_unit:"stk",icon:"🍅",description:"Produser og samle inn stekte tomater",image_key:"stekte-tomater"},
    {id:2,category:"Produksjon",name:"Varm sjokolade",amount:5,amount_unit:"stk",icon:"☕",description:"Produser og samle inn varm sjokolade",image_key:"varm-sjokolade"},
    {id:3,category:"Fiskeoppgave",name:"Fisk med sluk",amount:10,amount_unit:"kg",icon:"🎣",description:"Fang fisk med sluk",image_key:"fisk-med-sluk"},
    {id:4,category:"Produksjon",name:"Honningpopkorn",amount:3,amount_unit:"stk",icon:"🍿",description:"Produser og samle inn honningpopkorn",image_key:"honningpopkorn"},
    {id:5,category:"Produksjon",name:"Tomatjus",amount:3,amount_unit:"stk",icon:"🧃",description:"Produser og samle inn tomatjus",image_key:"tomatjus"},
    {id:6,category:"Produksjon",name:"Egg og bacon",amount:4,amount_unit:"stk",icon:"🍳",description:"Produser og samle inn egg og bacon",image_key:"egg-og-bacon"},
    {id:7,category:"Produksjon",name:"Grønn smoothie",amount:2,amount_unit:"stk",icon:"🥤",description:"Produser og samle inn grønn smoothie",image_key:"gronn-smoothie"},
    {id:8,category:"Produksjon",name:"Kake med røde bær",amount:3,amount_unit:"stk",icon:"🎂",description:"Produser og samle inn kake med røde bær",image_key:"kake-med-rode-baer"},
    {id:9,category:"Produksjon",name:"Bakt potet",amount:3,amount_unit:"stk",icon:"🥔",description:"Produser og samle inn bakt potet",image_key:"bakt-potet"},
    {id:10,category:"Innhøsting",name:"Ingefær",amount:85,amount_unit:"stk",icon:"🫚",description:"Høst inn fra ingefæråkrene",image_key:"ingefaer"},
    {id:11,category:"Besøkende i byen",name:"Danser",amount:1,amount_unit:"stk",icon:"💃",description:"Betjen danser",image_key:"danser"},
    {id:12,category:"Bybyggeoppgave",name:"Gjester i Matbutikk",amount:2,amount_unit:"stk",icon:"🏪",description:"Ta imot byggjester i Matbutikk",image_key:"gjester-i-matbutikk"}
  ];
  ...
  const BUNNY_PNG_IMAGE_KEYS=new Set(["rustikk-bukett", "dame", "ris", "popkorn-med-smor", "genser", "bygjester-kafe", "bjornebaer-muffins", "olivenolje", "sukkerror"]);
  ...
  function bunnyAmountLabel(task){
    const amount=Number(task?.amount)||1;
    return String(task?.amount_unit||"stk").toLowerCase()==="kg"?`${amount} kg`:`× ${amount}`;
  }
  ...
  function unusedTaskCount(result) {
    return Math.max(0, Number(result?.tasks_used || 0) - Number(result?.tasks_completed || 0));
  }

  function resultBaseMaximum(result) {
    const stored = Number(result?.possible_points || 0);
    if (stored > 0) return stored;
    return Math.max(0, Number(result?.included_tasks || 0) * Number(result?.points_per_task || 0));
  }

  function resultPercentValue(result) {
    const baseMaximum = resultBaseMaximum(result);
    const pointsEarned = Math.max(0, Number(result?.points_earned || 0));
    const stored = Number(result?.result_percent);
    const calculated = baseMaximum > 0 ? pointsEarned * 100 / baseMaximum : stored;
    const source = Number.isFinite(stored) ? stored : calculated;
    return Math.round(Math.min(100, Math.max(0, source || 0)) * 100) / 100;
  }

  function resultStatusModel(result) {
    const baseMaximum = resultBaseMaximum(result);
    const pointsEarned = Math.max(0, Number(result?.points_earned || 0));
    const pointsPerTask = Math.max(0, Number(result?.points_per_task || 0));
  ...
  function renderDerbyHistory() {
    ...
    const myAverage = ownRows.length ? ownRows.reduce((sum,row)=>sum+resultPercentValue(row),0)/ownRows.length : 0;
    ...
    if (canViewLeadership) {
      ...
      const leaders=[...groups.values()].map(rows=>{
        ...
        const capped=rows.map(resultPercentValue);
        return {
      ...
      }).sort((a,b)=>b.average-a.average||b.stars-a.stars||a.name.localeCompare(b.name,"nb"));
      ...
      if(leaderTable)leaderTable.innerHTML=leaders.length?leaders.map(item=>{
        ...
        return `<tr><td><strong>${esc(item.name)}</strong></td><td>${item.count}</td><td>${historyPercent(item.average)}</td><td>${item.perfect}</td><td>${item.stars}</td><td>${item.below}</td><td>${item.unused}</td><td><span class="history-trend ${item.trend>0?"up":item.trend<0?"down":"flat"}">${trend}</span></td></tr>`;
      }).join(""):`<tr><td colspan="8" class="empty-state">${currentLanguage==="en"?"No member results registered.":"Ingen medlemsresultater er registrert."}</td></tr>`;
    }
  ...
  function updateResultEditorCalculations() {
    ...
    $$(".result-member-editor").forEach(card=>{
      ...
      const possible=included*pointsPerTask;
      const result=points===null?null:resultStatusModel({included_tasks:included,extra_tasks:extra,tasks_used:available,tasks_completed:completed||0,points_per_task:pointsPerTask,points_earned:points,possible_points:possible});
      const unused=completed===null?0:Math.max(0,available-completed);
      ...
      const targetNote=!result||completed===null?"":result.star
        ...
        : result.percent>=100&&extra>0
          ...
          : result.percent<100
            ? ` · ${historyNumber(result.missingToMaximum)} ${currentLanguage==="en"?"points missing for 100%":"poeng mangler til 100 %"}`
            : "";
  ...
  if ("serviceWorker" in navigator) {
    window.addEventListener("load", () => navigator.serviceWorker.register("service-worker.js?v=0.18.0.88").catch(console.error));
