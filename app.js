(function () {
  ...
  if (profileChip) {
    profileChip.onclick = e => {
      e.preventDefault();
      e.stopPropagation();
      openOwnProfileHub();
    };
    let profilePointerHandled = false;
    profileChip.addEventListener("pointerup", e => {
      if (e.pointerType && e.pointerType !== "touch" && e.pointerType !== "pen") return;
      e.preventDefault();
      e.stopPropagation();
      profilePointerHandled = true;
      openOwnProfileHub();
      setTimeout(() => { profilePointerHandled = false; }, 250);
    }, {passive:false});
    profileChip.addEventListener("touchend", e => {
      if (profilePointerHandled) return;
      e.preventDefault();
      e.stopPropagation();
      openOwnProfileHub();
    }, {passive:false});
